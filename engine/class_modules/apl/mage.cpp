...
void arcane( player_t* p )
{
  ...
  precombat->add_action( "variable,name=mot_preceed_totm_by,op=reset,default=3" );
  ...
  fishing_opener->add_action( "arcane_missiles,if=buff.clearcasting.react&buff.clearcasting.stack=buff.clearcasting.max_stack&covenant.venthyr&cooldown.mirrors_of_torment.ready", "If we're at max stacks of CC and we haven't used MoT yet, use a stack to prevent munching" );
  ...
  fishing_opener->add_action( "mirrors_of_torment,if=buff.rune_of_power.down|prev_gcd.1.arcane_barrage" );
  fishing_opener->add_action( "touch_of_the_magi,if=buff.rune_of_power.down|prev_gcd.1.arcane_barrage|prev_gcd.1.mirrors_of_torment|prev_gcd.1.radiant_spark|prev_gcd.1.deathborne" );
  ...
  fishing_opener->add_action( "arcane_barrage,if=cooldown.arcane_power.ready&mana.pct<(40+(10*covenant.kyrian))&buff.arcane_charge.stack=buff.arcane_charge.max_stack&!runeforge.siphon_storm&!runeforge.temporal_warp", "We want to make sure we have enough mana for the entire AP window, even if there's still time remaining on RoP. It's only better to burn to zero and Evo if running the SS or TW legendaries." );
  fishing_opener->add_action( "arcane_barrage,if=buff.arcane_power.up&buff.arcane_power.remains<=gcd&(runeforge.arcane_infinity|cooldown.evocation.remains)", "Unless running the Arcane Harmony legendary, Barrage should not be used at the end of AP if Evocation is off cooldown as dropping charges will make it take longer to burn through remaining mana" );
  fishing_opener->add_action( "arcane_barrage,if=buff.rune_of_power.up&buff.arcane_power.down&buff.rune_of_power.remains<=gcd", "Barrage should always be the last cast in the RoP window as TotM will restore the charges spent" );
  ...
  fishing_opener->add_action( "arcane_missiles,if=buff.clearcasting.react&cooldown.arcane_power.remains&(buff.rune_of_power.up|buff.arcane_power.up),chain=1", "Only start using CC AM when AP has been used" );
  ...
  cooldowns->add_action( "mirrors_of_torment,if=cooldown.touch_of_the_magi.remains<variable.mot_preceed_totm_by|(cooldown.arcane_power.remains>variable.mot_max_delay_for_ap&cooldown.touch_of_the_magi.remains>variable.mot_max_delay_for_totm)", "Pair MoT with AP if it's coming up soon. If it will be used with TotM, cast it slightly before" );
  ...
  rotation->add_action( "arcane_barrage,if=buff.arcane_power.up&buff.arcane_power.remains<=gcd&buff.arcane_charge.stack=buff.arcane_charge.max_stack&(cooldown.evocation.remains|runeforge.arcane_infinity)" );
  rotation->add_action( "arcane_barrage,if=buff.rune_of_power.up&buff.arcane_power.down&buff.rune_of_power.remains<=gcd&buff.arcane_charge.stack=buff.arcane_charge.max_stack&(cooldown.evocation.remains|runeforge.arcane_infinity)" );
...
void fire( player_t* p )
{
  ...
  default_->add_action( "frostbolt,if=runeforge.disciplinary_command&cooldown.buff_disciplinary_command.remains<cast_time&buff.disciplinary_command_frost.down&!buff.disciplinary_command.up&(variable.time_to_combustion>cooldown.buff_disciplinary_command.duration|variable.time_to_combustion<5)" );
  ...
  combustion_timing->add_action( "variable,name=combustion_ready_time,value=cooldown.combustion.remains*(1-(variable.kindling_reduction+0.2*firestarter.active)*talent.kindling)", "Helper variable that contains the actual estimated time that the next Combustion will be ready." );
  ...
  combustion_timing->add_action( "variable,name=combustion_time,op=max,value=cooldown.buff_disciplinary_command.duration+cooldown.buff_disciplinary_command.remains,if=cooldown.buff_disciplinary_command.remains+cooldown.buff_disciplinary_command.duration<variable.combustion_time-10", "Delay Combustion by up to 10 seconds to fit in extra usages of Disciplinary Command." );
  ...
  combustion_timing->add_action( "variable,name=combustion_time,value=variable.combustion_ready_time,if=variable.combustion_ready_time+cooldown.combustion.duration*(1-(0.2+variable.kindling_reduction+0.2*firestarter.active)*talent.kindling)<=variable.combustion_time|variable.combustion_time>fight_remains-20", "Use the next Combustion on cooldown if it would not be expected to delay the scheduled one or the scheduled one would happen less than 15 seconds before the fight ends. Add to variable.kindling_reduction because this includes the frontloaded triggers during Combustion and that variable is meant to be used after Combustion." );
